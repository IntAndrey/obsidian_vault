...
ОбщийМодуль ОборудованияЧекопечатающиеУстройстваКлиент.Модуль
стр. 18
//Для печати на принтере чеков AVI
			
	ПараметрыВыполнениеКоманды.СообщенияВСлужбуТехническойПоддержки = Истина;
	Если ПараметрыОперации.АдресРасчетов = "" Тогда 
		   ПараметрыОперации.Вставить("АдресРасчетов","Хабаровск");
		   ПараметрыОперации.Вставить("МестоРасчетов","Магазин Семья");
	КонецЕсли;
...
Нужно самостоятельно формировать данные для печати
Номер смены и время смены
Шаблон чека
Для чекопечатающих устройств


ОбщийМодуль ОборудованияЧекопечатающиеУстройства 
стр. 2904 
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт

Вызывается передачей имени модуля в текстовом виде
